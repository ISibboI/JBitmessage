...
/**
 * MpRSASSA-PSS (MpRSA Signature Scheme with Appendix - Probabilistic Signature
 * Scheme) is an asymmetric signature scheme with appendix combining the MpRSA
 ...
 */
public class MpRSASignaturePSS extends Signature {
     ...
     * object for MpRSASSA-PSS with OID 1.2.840.113549.1.1.10. Failure to
     * achieve this is fatal and will result in a <code>RuntimeException</code>
     */
    public MpRSASignaturePSS() {
     ...
     *                 if the key is not an instance of MpRSAPrivateKey
     */
	...
	if (!(privateKey instanceof MpRSAPrivateKey)) {
	    throw new InvalidKeyException(
		    "key is not an instance of MpRSAPrivateKey");
	}

	privKey = (MpRSAPrivateKey) privateKey;
	...
	// 2) MpRSA verification:
	// a) Convert the signature S to an integer signature representative s:
	s = PKCS1Operations.OS2IP(signature);

	try {
	    // b) Apply the MpRSAVP1 (equivalent to MpRSAEP) verification
	    // primitive to the MpRSA public key (n, e) and the signature
	    // representative s to produce an integer message representative m =
	    // MpRSAVP1 ((n, e), s).
	    // *) If MpRSAVP1 outputs "signature representative out of range",
	    // output "invalid signature" and stop.
	    m = MpRSAOperations.MpRSAEP(pubKey, s);
	    ...
	    // length in bits of the MpRSA modulus n: EM = I2OSP (m, emLen)
	    ...
	    // *) MpRSAEP throws PKCS1Execption if signature representative out
	    // of range.
